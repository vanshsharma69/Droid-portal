...
export const api = {
  ...
  getProjects: (token) => request("/api/projects", { token }),
  getProject: (id, token) => request(`/api/projects/${id}`, { token }),
  createProject: (payload, token) =>
    request("/api/projects", { method: "POST", data: payload, token }),
  updateProject: (id, payload, token) =>
    request(`/api/projects/${id}`, { method: "PUT", data: payload, token }),
  deleteProject: (id, token) => request(`/api/projects/${id}`, { method: "DELETE", token }),
